...
var SEIUserManager = class {
  ...
  constructor(options) {
    ...
    this.storeFile = (0, import_path.join)(this.storagePath, "sei-users.encrypted.json");
  }
  /** Inicializa o gerenciador */
  async init() {
    if (!(0, import_fs.existsSync)(this.storagePath)) {
      await (0, import_promises.mkdir)(this.storagePath, { recursive: true });
    }
    if ((0, import_fs.existsSync)(this.storeFile)) {
...
var SEIService = class extends import_events2.EventEmitter {
  ...
  async init() {
    ...
    await (0, import_promises2.mkdir)((0, import_path2.join)(this.config.dataPath, "downloads"), { recursive: true });
  }
  // ============================================
  // Gestão de Usuários
  // ============================================
  ...
  async downloadDocuments(session, user, docs) {
    const downloadPath = (0, import_path2.join)(this.config.dataPath, "downloads", user.id);
    ...
    for (const doc of docs) {
      try {
        ...
        const download = await downloadPromise;
        const filePath = (0, import_path2.join)(downloadPath, doc.nome);
  ...
  async downloadProcess(session, user, item) {
    try {
      ...
      const download = await downloadPromise;
      const filePath = (0, import_path2.join)(downloadPath, `Processo_${item.numero?.replace(/[^0-9]/g, "") ?? item.id}.pdf`);
      await download.saveAs(filePath);
      return filePath;
    } catch {
      return void 0;
