...
var import_agentkeepalive, import_abort_controller, import_node_fs, import_node_stream, import_web, fileFromPathWarned, defaultHttpAgent, defaultHttpsAgent;
...
__export(index_exports, {
  SEIBrowserClient: () => SEIBrowserClient,
  SEIClient: () => SEIClient,
  SEIDaemon: () => SEIDaemon,
  SEINotificationService: () => SEINotificationService,
  SEIService: () => SEIService,
  SEIServiceAPI: () => SEIServiceAPI,
  SEISoapClient: () => SEISoapClient,
  SEIUserManager: () => SEIUserManager,
  SEIWatcher: () => SEIWatcher,
  SEI_SELECTORS: () => SEI_SELECTORS,
  SelectorStore: () => SelectorStore,
  classifyError: () => classifyError,
  createAgentFallback: () => createAgentFallback,
  decrypt: () => decrypt,
  ...
  resolveResilienceConfig: () => resolveResilienceConfig,
  withRetry: () => withRetry
});
...
async function withRetry(fn, opts) {
  ...
  for (let attempt = 0; attempt <= opts.maxRetries; attempt++) {
    try {
      ...
      const kind2 = classifyError(error);
      if (!retryOn.includes(kind2)) break;
...
function createAgentFallback(config) {
  ...
  async function getClient() {
    ...
    try {
      ...
      anthropicClient = new Anthropic2({ apiKey });
...
Analise o screenshot e o DOM para encontrar o elemento correto.`;
        ...
        const text = response.content.filter((c2) => c2.type === "text").map((c2) => c2.text).join("");
...
async function extractInteractiveDOM(page) {
  return await page.evaluate(() => {
    ...
    for (let i2 = 0; i2 < roleNodes.length; i2++) {
      const el = roleNodes[i2];
      if (el.offsetParent !== null) {
...
var SEIBrowserClient = class {
  ...
  async init() {
    ...
    if (options.cdpEndpoint) {
      this.browser = await import_playwright.chromium.connectOverCDP(options.cdpEndpoint);
  ...
  async clickSmart(target, aria, cssFallback) {
    const t2 = this.resilience.failFastTimeout;
  ...
  async waitForSmart(target, aria, cssFallback, options) {
    const t2 = options?.timeout ?? this.resilience.failFastTimeout;
    const state = options?.state ?? "visible";
    const storeKey = this.getSelectorKey(aria.role, aria.name);
    try {
      const locator = aria.name ? target.getByRole(aria.role, { name: aria.name }) : target.getByRole(aria.role);
      await failFast(() => locator.first().waitFor({ timeout: t2, state }), t2 + 500);
      this.selectorStore.recordSuccess(storeKey);
      return;
    } catch {
    }
    if (cssFallback) {
      ...
      await locator.first().waitFor({ timeout: t2, state });
      return;
    }
    const cached = this.selectorStore.get(storeKey);
    if (cached) {
        ...
        try {
          ...
          if (select) {
            const optionValues = await select.$$eval(
              "option",
              (opts) => opts.map((o2) => ({ value: o2.value, text: o2.textContent }))
            );
            const match = optionValues.find(
              (o2) => o2.text?.toLowerCase().includes(options.tipoProcedimento.toLowerCase())
            );
            if (match?.value) {
  ...
  async uploadDocument(nomeArquivo, conteudoBase64, options = {}) {
    ...
    await fs4.writeFile(tempPath, buffer);
    ...
    try {
      ...
      if (selectEl) {
        const options = await selectEl.$$eval(
          "option",
          (opts) => opts.filter((o2) => o2.value).map((o2) => ({ id: o2.value, nome: o2.textContent?.trim() ?? "" }))
        );
        return options;
      }
    } catch {
  ...
  async listUnits() {
    ...
    try {
      ...
      if (selectEl) {
        const options = await selectEl.$$eval(
          "option",
          (opts) => opts.filter((o2) => o2.value).map((o2) => ({
            id: o2.value,
            sigla: o2.textContent?.trim() ?? "",
            descricao: o2.textContent?.trim() ?? ""
          }))
        );
        return options;
      }
  ...
  async atribuirProcesso(numeroProcesso, nomeUsuario) {
    ...
    try {
      ...
      if (select) {
        ...
        const match = options.find((o2) => o2.text?.toLowerCase().includes(nomeUsuario.toLowerCase()));
        if (match?.value) {
    ...
    try {
      ...
      if (selectEl) {
        const options = await selectEl.$$eval(
          ...
          (opts) => opts.filter((o2) => o2.value).map((o2) => ({
            id: o2.value,
            nome: o2.textContent?.trim() ?? "",
            sigla: o2.value
          }))
        );
        if (filter) {
          return options.filter((u2) => u2.nome.toLowerCase().includes(filter.toLowerCase()));
        }
        return options;
      }
    } catch {
  ...
  async downloadProcess(numeroProcesso, _includeAttachments = true, outputPath) {
    ...
    const fs4 = await import("fs/promises");
    const stats = await fs4.stat(suggestedPath);
  ...
  async listAndamentos(numeroProcesso, options) {
    if (this.soapAvailable && this.soapClient && this.currentIdUnidade) {
      try {
        ...
        return andamentos.map((a2) => ({
          data: a2.DataHora,
          unidade: a2.Unidade?.Sigla ?? "",
          usuario: a2.Usuario?.Nome ?? "",
    ...
    if (this.browserClient) {
      ...
      if (details) {
        return {
          ...
          UnidadesProcedimentoAberto: details.unidadesAbertas.map((u2) => ({ IdUnidade: "", Sigla: u2, Descricao: u2 })),
          Assuntos: [],
          Interessados: details.interessados.map((i2) => ({ Sigla: i2, Nome: i2 }))
        };
      }
    }
    throw new Error("Nenhum cliente dispon\xEDvel");
    ...
    if (this.soapAvailable && this.soapClient && this.currentIdUnidade) {
      try {
        return await this.soapClient.gerarProcedimento(this.currentIdUnidade, {
          ...
          Assuntos: options.assuntos.map((a2) => ({ CodigoEstruturado: a2 })),
          Interessados: options.interessados?.map((i2) => ({ Sigla: i2, Nome: i2 })),
  ...
  async createDocument(numeroProcesso, options) {
    if (this.soapAvailable && this.soapClient && this.currentIdUnidade) {
      const doc = await this.soapClient.incluirDocumento(this.currentIdUnidade, {
        ...
        Interessados: options.interessados?.map((i2) => ({ Sigla: i2, Nome: i2 })),
        Destinatarios: options.destinatarios?.map((d2) => ({ Sigla: d2, Nome: d2 })),
        Observacao: options.observacao,
        NivelAcesso: options.nivelAcesso ?? 0,
        IdHipoteseLegal: options.hipoteseLegal,
    ...
    if (this.soapAvailable && this.soapClient && this.currentIdUnidade) {
      try {
        ...
        return {
          ...
          tipo: doc.Serie?.Nome ?? "",
          data: doc.Data,
          assinaturas: doc.Assinaturas?.map((a2) => ({
            nome: a2.Nome,
            cargo: a2.CargoFuncao,
            data: a2.DataHora
          })) ?? []
        };
      } catch (err) {
    ...
    if (this.soapAvailable && this.soapClient && this.currentIdUnidade) {
      try {
        ...
        return {
          ...
          documentos: bloco.Documentos?.map((d2) => d2.IdProtocolo) ?? []
        };
      } catch (err) {
...
var SEIWatcher = class extends import_events.EventEmitter {
  client;
  options;
  intervalId = null;
  isRunning = false;
...
function deriveKey(password, salt) {
  return (0, import_crypto.scryptSync)(password, salt, KEY_LENGTH);
}
...
function generateSecurePassword(length = 32) {
  return (0, import_crypto.randomBytes)(length).toString("base64").slice(0, length);
}
...
var SEIUserManager = class {
  ...
  constructor(options) {
    ...
    this.storeFile = (0, import_path2.join)(this.storagePath, "sei-users.encrypted.json");
  }
  /** Inicializa o gerenciador */
  async init() {
    if (!(0, import_fs2.existsSync)(this.storagePath)) {
      await (0, import_promises.mkdir)(this.storagePath, { recursive: true });
    }
    if ((0, import_fs2.existsSync)(this.storeFile)) {
      const data = await (0, import_promises.readFile)(this.storeFile, "utf-8");
  ...
  async save() {
    ...
    await (0, import_promises.writeFile)(this.storeFile, JSON.stringify(this.store, null, 2));
  }
  /** Adiciona um novo usuário */
  async addUser(options) {
    if (!this.store) throw new Error("Store n\xE3o inicializado");
    const existing = this.store.users.find((u2) => u2.config.id === options.id);
    if (existing) {
  ...
  async updateUser(id, updates) {
    if (!this.store) throw new Error("Store n\xE3o inicializado");
    const userIndex = this.store.users.findIndex((u2) => u2.config.id === id);
    if (userIndex === -1) {
  ...
  getUser(id) {
    if (!this.store) throw new Error("Store n\xE3o inicializado");
    const user = this.store.users.find((u2) => u2.config.id === id);
  ...
  getAllUsers() {
    if (!this.store) throw new Error("Store n\xE3o inicializado");
    return this.store.users.map((u2) => u2.config);
  }
  /** Lista usuários ativos */
  getActiveUsers() {
    return this.getAllUsers().filter((u2) => u2.active);
  }
...
var SEINotificationService = class {
  ...
  buildSubject(payload) {
    ...
    switch (payload.type) {
      ...
      case "prazos":
        const urgentes = payload.items.filter((i2) => i2.prazo?.status === "vencido" || i2.prazo?.status === "vencendo_hoje").length;
        if (urgentes > 0) {
...
// src/service.ts
var SEIService = class extends import_events2.EventEmitter {
  config;
  userManager;
  notifier;
  ...
  async init() {
    ...
    await (0, import_promises2.mkdir)((0, import_path3.join)(this.config.dataPath, "downloads"), { recursive: true });
  }
  // ============================================
  // Gestão de Usuários
  // ============================================
  ...
  async enrichItems(session, user, event) {
    ...
    for (const item of event.items) {
      ...
      try {
        ...
        enrichedItem.documentos = docs.filter((d2) => d2.tipo && d2.titulo).slice(0, 10).map((d2) => ({
          id: d2.id,
          nome: d2.titulo,
          tipo: d2.tipo,
          data: hoje
  ...
  async downloadDocuments(session, user, docs) {
    const downloadPath = (0, import_path3.join)(this.config.dataPath, "downloads", user.id);
    await (0, import_promises2.mkdir)(downloadPath, { recursive: true });
    for (const doc of docs) {
      try {
        ...
        const download = await downloadPromise;
        const filePath = (0, import_path3.join)(downloadPath, doc.nome);
  ...
  async downloadProcess(session, user, item) {
    try {
      ...
      const download = await downloadPromise;
      const filePath = (0, import_path3.join)(downloadPath, `Processo_${item.numero?.replace(/[^0-9]/g, "") ?? item.id}.pdf`);
      await download.saveAs(filePath);
      return filePath;
    } catch {
      return void 0;
...
var SEIDaemon = class extends import_events3.EventEmitter {
  config;
  client = null;
  watcher = null;
  notifier = null;
...
var SEIServiceAPI = class {
  ...
  async start() {
    ...
    this.server = (0, import_http2.createServer)(async (req, res) => {
  ...
  async handleListUnits(client, query) {
    ...
    const filtered = query.filter ? unidades.filter((u2) => {
      const filterLower = query.filter.toLowerCase();
      return u2.Sigla.toLowerCase().includes(filterLower) || u2.Descricao.toLowerCase().includes(filterLower);
    }) : unidades;
